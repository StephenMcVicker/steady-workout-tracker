...
const BACKUP_FORMAT="steady-workout-backup-v1";
...
export default function Home(){
 ...
 function exportBackup(){
  const backup={app:"Steady",format:BACKUP_FORMAT,exportedAt:new Date().toISOString(),store};
  const url=URL.createObjectURL(new Blob([JSON.stringify(backup,null,2)],{type:"application/json"}));
  const link=document.createElement("a");link.href=url;link.download=`steady-backup-${localDate()}.json`;link.click();window.setTimeout(()=>URL.revokeObjectURL(url),1000);
 }
 function importBackup(event:ChangeEvent<HTMLInputElement>){
  const file=event.target.files?.[0];event.target.value="";if(!file)return;
  if(file.size>5*1024*1024){setNotice("That backup is too large to import.");return;}
  const reader=new FileReader();
  reader.onload=()=>{
   try{
    const backup=JSON.parse(String(reader.result)) as {format?:unknown;store?:unknown};
    if(backup.format!==BACKUP_FORMAT||!isStore(backup.store))throw new Error("This file isn’t a valid Steady backup.");
    if(!window.confirm("Import this backup and replace the workout data currently saved on this device?"))return;
    setStore(backup.store);setStorageError("");setDraftDate(localDate());setDetail(null);setView("home");setNotice("Backup restored. Your workouts are back.");
   }catch(error){setNotice(error instanceof Error?error.message:"Couldn’t read that backup file.");}
  };
  reader.onerror=()=>setNotice("Couldn’t read that backup file.");reader.readAsText(file);
 }
 function finish(){
 ...
 const nav=(mobile=false)=><nav className={mobile?"bottom-nav":"top-nav"} aria-label={mobile?"Mobile navigation":"Main navigation"}><button className={view==="home"||view==="workout"?"selected":""} aria-current={view==="home"||view==="workout"?"page":undefined} onClick={()=>{setView("home");setDetail(null);}}><Dumbbell size={19}/>Workouts</button><button className={view==="history"||view==="summary"?"selected":""} aria-current={view==="history"||view==="summary"?"page":undefined} onClick={()=>{setView("history");setDetail(null);}}><History size={19}/>History</button></nav>;
 ...
 const screenKey=`${view}-${category}-${exercise?.id??""}-${view==="workout"&&completed===total?"complete":""}`;
 return <div className="app-shell">
  ...
  <main className="main-content">
   ...
   <div className="screen-transition" key={screenKey}>{view==="home"?<>
    ...
    <div className="workout-progress-inline"><div><span>Progress</span><strong>{completed} / {total} sets</strong></div><Progress value={completed/total*100} aria-label="Workout progress"/></div>
...
function summarizeSets(e:Exercise,sets:{kg:number;reps:number;done:boolean}[]){
 const formatWeight=(kg:number)=>kg===0&&e.id==="lunge"?"bodyweight":`${kg} kg${e.unit.includes("total")?" total":e.unit.startsWith("per dumbbell")?" each":""}`;
 const first=sets[0];
 if(sets.every(set=>set.kg===first.kg&&set.reps===first.reps))return `${sets.length} × ${first.reps}${e.id==="lunge"?" / leg":""} @ ${formatWeight(first.kg)}`;
 return sets.map((set,index)=>`Set ${index+1}: ${set.reps}${e.id==="lunge"?" / leg":""} @ ${formatWeight(set.kg)}`).join(" · ");
}
...
function ExerciseDetail({exercise:e,sets,onBack,onSave,history}:{exercise:Exercise;sets:{kg:number;reps:number;done:boolean}[];onBack:()=>void;onSave:(index:number,kg:number,reps:number,done:boolean)=>void;history:Session[]}){
 ...
 const lastSession=[...history].sort((a,b)=>b.startedAt.localeCompare(a.startedAt)).find(s=>s.exercises[e.id]?.some(set=>set.done));
 const lastSets=lastSession?.exercises[e.id]?.filter(set=>set.done)??[];
 useEffect(()=>{if(restSeconds===null)return;const timer=window.setInterval(()=>setRestSeconds(value=>value===null||value<=1?null:value-1),1000);return()=>window.clearInterval(timer);},[restSeconds!==null]);
 function completeCurrentSet(){onSave(selected,Number(kg),Number(reps),true);const following=sets.findIndex((set,i)=>i!==selected&&!set.done);if(following!==-1){choose(following);setRestSeconds(75);}}
