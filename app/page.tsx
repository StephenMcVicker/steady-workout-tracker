...
export default function Home(){
 ...
 function updateSet(id:string,index:number,kg:number,reps:number,done:boolean){
  ...
  if(done){setDetail(null);setNotice("Set saved. Rest 60–90 seconds.");}
 }
 function finish(){
 ...
 return <div className="app-shell">
  <header className="site-header"><div className="header-inner"><a href={assetPath("/")} className="brand" aria-label="Steady home"><span className="brand-mark"><Dumbbell size={21}/></span>steady<span className="brand-period">.</span></a>{nav()}<span className="local-label"><Smartphone size={16}/>On your device</span></div></header>
  <main className="main-content">
   ...
   {view==="home"?<>
    ...
    <div className="workout-layout"><section className="exercise-section" aria-label={`${category} exercises`}><div className="exercise-grid">{exercises.map((e,index)=>{const done=session?.exercises[e.id]?.filter(s=>s.done).length??0;const locked=done===e.sets;return <button key={e.id} className={`exercise-card ${locked?"is-complete":""}`} onClick={()=>openExercise(e)} disabled={!ready||locked} aria-label={`${e.name}, ${done} of ${e.sets} sets complete${locked?", locked":""}`}><div className="exercise-image"><img src={assetPath(`/exercises/${e.image}-1.jpg`)} alt={`${e.name} reference${e.photoNote?" — equipment variation":""}`} width={600} height={400}/><span className="exercise-number">{String(index+1).padStart(2,"0")}</span><span className={`equipment-tag ${e.photoNote?"variation-tag":""}`}>{e.id==="squat"?"Kettlebell demo":e.id==="rdl"?"Barbell demo":"Dumbbells"}</span>{locked&&<span className="complete-badge"><Check size={16}/>Complete</span>}</div><div className="card-content"><p className="muscle-label">{e.muscle}</p><h3>{e.name}</h3><div className="prescription"><span>{e.sets} sets <span className="muted-dot">·</span> {e.reps} reps{e.id==="lunge"?" / leg":""}</span><span>{session?.exercises[e.id]?.find(s=>!s.done)?.kg??store.weights[e.id]??e.kg} kg</span></div><div className="card-footer"><span className="set-dots" aria-hidden="true">{Array.from({length:e.sets},(_,i)=><span key={i} className={session?.exercises[e.id]?.[i].done?"done":""}/>)}</span><span>{locked?"All done":done?`${done} of ${e.sets} sets`:"View exercise"}{locked?<LockKeyhole size={14}/>:<ChevronRight size={16}/>}</span></div></div></button>;})}</div></section><aside className="session-column"><section className="session-panel"><span className="panel-icon"><Dumbbell size={23}/></span><p className="eyebrow">YOUR SESSION</p><h2>{completed===total?"You did that.":"One set closer."}</h2><div className="session-progress-block"><div className="progress-label"><span>Sets complete</span><strong>{completed}<span> / {total}</span></strong></div><Progress value={completed/total*100} aria-label="Workout completion"/></div><DatePicker value={chosenDate} onChange={changeDate}/><Button className="primary-action" onClick={()=>completed===total?finish():openExercise(exercises.find(e=>session?.exercises[e.id].some(s=>!s.done))??exercises[0])}>{completed===total?<><Check/>Finish session</>:<>Next exercise<ArrowUpRight/></>}</Button><p className="autosave">{storageError?"Saving unavailable":"Your progress saves automatically."}</p></section><div className="mini-tip"><Leaf size={20}/><p>Rest 60–90 seconds between sets.</p></div></aside></div>
   </>}
